...
variable "app_bucket_arn" {
  type        = string
  description = "ARN of the S3 bucket app pods read/write scoped data to"
}

variable "devops_cluster_sg_id" {
  type        = string
  description = "DevOps cluster security group ID — allowed inbound to K3s API (6443) so ArgoCD can deploy here"
}
