...
| UI access (Woodpecker, ArgoCD, Grafana) | SSM API tunnel + kubectl port-forward | `./scripts/tunnel.sh dev` opens K3s API tunnel (6443) then forwards localhost:8080→ArgoCD, 8081→Woodpecker, 8082→Grafana. No /etc/hosts. No Traefik routing needed. |
...
All DevOps tooling runs in private subnets — never exposed to the internet. Access uses SSM port-forwarding:

```
You (laptop)
   │  ./scripts/tunnel.sh dev
   │  (SSM port-forward 8080 → Traefik port 80)
   ▼
Traefik (K3s built-in ingress)
   ├── woodpecker.devops → Woodpecker CI  → GitHub OAuth
   ├── argocd.devops     → ArgoCD         → GitHub SSO + RBAC
   └── grafana.devops    → Grafana        → GitHub OAuth

kubectl / helm (bootstrap + occasional use)
   │  SSM port-forward → K3s API (port 6443)
   ▼
DevOps cluster or App cluster
```

**Future:** Tailscale operator would make UIs always-on (no tunnel command needed). Deferred until a public domain is available for Woodpecker webhook callbacks.
